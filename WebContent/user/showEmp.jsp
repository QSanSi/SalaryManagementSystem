...
<script type="text/javascript">		
	$(document).ready(function(){		
		...
		$("#go").click(function(){
			...
			$("#go").attr("href","emppageSelectEmp.action?page=" + number + "&empId=" + i + "&trueName=" + n + "&dept=" + d);
	      }); 
		$("#first").click(function(){
			var i = $("#empid").val();
			var n = $("#truename").val();
			var d = $("#dept").val();
			$("#first").attr("href","emppageSelectEmp.action?page=1&empId=" + i + "&trueName=" + n + "&dept=" + d);
	      }); 
		$("#pre").click(function(){
			var i = $("#empid").val();
			var n = $("#truename").val();
			...
			$("#pre").attr("href","emppageSelectEmp.action?page=" + number + "&empId=" + i + "&trueName=" + n + "&dept=" + d);
	      }); 
		$("#next").click(function(){
			var i = $("#empid").val();
			var n = $("#truename").val();
			var d = $("#dept").val();
			var number = "${requestScope.page+1}";
			$("#next").attr("href","emppageSelectEmp.action?page=" + number + "&empId=" + i + "&trueName=" + n + "&dept=" + d);
	      }); 
		$("#last").click(function(){
			var i = $("#empid").val();
			var n = $("#truename").val();
			...
			$("#last").attr("href","emppageSelectEmp.action?page=" + number + "&empId=" + i + "&trueName=" + n + "&dept=" + d);
	      }); 
	});
</script>
	...
	<div class="table_top" style="height: 100%">
		...
		<div height="100%" style="margin-top: 10px;">
			...
			<div style="margin-bottom: 10px;">
				<s:form action="findEmp" method="post" theme="simple" name="f">
			工号：<s:textfield id="empid" name="empId" cssClass="input" />&nbsp;&nbsp;
			员工名称：<s:textfield id="truename" name="trueName" cssClass="input" />&nbsp;&nbsp;
			部门：<select id="dept" name="emp.deptName">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="showDept.action"> <img src="../skin/22.gif" />&nbsp;&nbsp;新增
					</a>&nbsp;&nbsp;
		</s:form>
			</div>
			<table border="0" cellpadding="0" cellspacing="1" bgcolor="b5d6e6">
				...
				<c:forEach items="${emps}" var="emp" varStatus="i">
					<tr class="list" bgcolor="#FFFFFF">
						...
						<td>${emp.deptName}</td>
						<td>${emp.startDate}</td>
						<td><a href="emp!load.action?emp.id=${emp.id}"> <img
								src="../skin/edt.gif" />修改
						</a> <a href="emp!delete.action?emp.id=${emp.id}"> <img
								src="../skin/del.gif" />删除
						</a></td>
					</tr>
				</c:forEach>
